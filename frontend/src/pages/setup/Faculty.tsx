...
export default function FacultyPage() {
  ...
  const handleEdit = (faculty: any) => {
    ...
    setNewName(faculty.name);
    setNewEmpId(faculty.employee_id);
    setNewMaxLoad(faculty.max_weekly_load);
    setNewExpertise(faculty.expertise?.join(", ") || "");
    setDrawerOpen(true);
  };
  ...
  const handleAdd = async () => {
    ...
    if (editingFaculty) {
      await update(editingFaculty.faculty_id, {
        name: newName,
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Faculty" description="Manage department faculty members">
        <Sheet open={drawerOpen} onOpenChange={(open) => { setDrawerOpen(open); if (!open) { setEditingFaculty(null); setNewName(""); setNewEmpId(""); setNewExpertise(""); setNewMaxLoad(20); } }}>
          ...
          <SheetContent className="w-[400px]">
            ...
            <div className="space-y-4 mt-6">
              <div className="space-y-2"><Label>Full Name</Label><Input className="rounded-xl" placeholder="Dr. Rajesh Patel" value={newName} onChange={(e) => setNewName(e.target.value)} /></div>
              ...
              <Button className="w-full rounded-xl btn-press" onClick={handleAdd}>{editingFaculty ? "Update Faculty" : "Add Faculty"}</Button>
            </div>
          </SheetContent>
        </Sheet>
      </PageHeader>
            ...
            return (
              <div key={f.faculty_id} className="bg-card rounded-xl shadow-sm p-5 card-hover group relative">
                <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
                  <Button variant="ghost" size="icon" className="rounded-xl h-7 w-7" onClick={() => handleEdit(f)}><Pencil className="h-3.5 w-3.5" /></Button>
